...
We first derive the energy required to both distribute ($E_{net}$) and process ($E_{node}$) oxygen in
biological networks. From basic principles of hydraulics, 
%the energy dissipated to transport a constant volume of blood through the network (
$E_{net}$ is
...
%N^{1-2/D_r}$.  Pipelining also implies that $T_{node} = T_{net}$. We already
%said this Following \cite{banavar10}, we assume that 'supply equals demand,'
%which 
...
Scaling analyses provide a powerful framework for understanding constraints on both biological and computational systems over enormous size ranges. Delong et al show that, contrary to previous understanding, metabolic scaling slopes and intercepts change at the evolutionary transitions: prokaryote (bacteria) metabolism is superlinear with size, unicellular protist metabolism is linear, and multicellular animal metabolism is sublinear, converging toward the canonical ? exponent. Delong et al hypothesize that shifts in the metabolic exponent result from organisms overcoming a series of constraints, and then facing new constraints, as they increase in size.
