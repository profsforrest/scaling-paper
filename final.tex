...
dimension of the system. For example in \red{Figure \ref{fig:firstfig} C}, where $\lambda = 4$ and $D_l = 2$, wires are $4^{1/2} = 2$ times longer when they connect to successively higher modules in the hierarchy.
...
prediction.  Note that Eq.~\ref{eq:OrganismPower} includes two
...
\caption{\red{The energy-time minimization model predicts how throughput
  scales with the number of transistors.  The raw data are includes as
  supplementary materials. The model's
...
presented a unified model, which predicts scaling relationships for both organisms and
...
observed curvature in metabolic scaling of organisms by including an
...
previous models.  The model is consistent with the results of a
previous study~\cite{delong2010shifts} at the evolutionary transitions. In this context,
...
\red{\paragraph{Chips are like bacteria:}} The chip scaling described above
...
\red{\paragraph{Chips are like protists:}} Historical chip scaling mimics the linear relationship between performance and
...
\red{\paragraph{Chips transition to multicellularity:}} Computer chips are currently undergoing
...
\red{\paragraph{Chips are not exactly like biology:}} There are also important differences between scaling of oxygen delivery in biology and information
...
\red{\paragraph{Decentralized designs and sociality:}}  We now
