...
microprocessors as composed of nodes (transistors that perform computation), communicating
...
for mammals (Sections \ref{sec:mammals} and \ref{sec:bio-predictions}) and then for computers (Section
...
plots, as shown in figure~\ref{fig:OrganismsPowerScaling}, it is important because the lack of a single scaling exponent calls into question prior scaling models that purport to explain a universal scaling exponent.}
...
\begin{figure}[!h] \centering
  ...
  \caption{\red{The energy-time minimization model predicts metabolic
    ...
    shown as a dashed green line ($D_r = 2.56$).}}
...
\subsection{Summary of time-energy minimization model predictions}
...
delivery time, e.g., \cite{west97,RIVER}.  Here we extend that work by considering minimization of energy and time
...
observed curvature in metabolic scaling of mammals.  Other studies have interpreted the
deviation from linear scaling as indicating that there is no single unified metabolic scaling theory.  The
framework presented here accounts for curvature by including time and energy minimization in the network and the nodes. In computation, the unified model accurately predicts Rent's exponents, active power consumption and chip performance in over 40 years of chip design.
...
\red{Innovations in chip design mimic innovation in the evolution of bacteria:} The chip scaling described above
