...
Here, we model mammals as composed of regions of tissue (service
...
optimal network design as a trade-off between energy cost and processing speed. This energy-time framework is consistent with the major
...
for mammals (Sections \ref{sec:organisms} and \ref{sec:bio-predictions}) and then for computers (Section
...
Similar to the length scaling of the service volume in mammals, the
...
proportionally with decreasing transistor size~\cite{moses08}. Thus,
	service regions are \emph{smaller} in more powerful chips (which have more transistors), but they are \emph{larger} in larger animals.
\end{enumerate}
...
\section{Predictions of the Model for Mammals and Computers}
...
both mammals and computers we define the total energy as the sum of energy
...
(Sections~\ref{sec:mammals} and \ref{sec:bio-predictions} ) and computers (Section~\ref{sec:computers}) in terms of
...
network and nodes in mammals, and then use them to minimize Eq.
...
We make a specific prediction for the value of $D_r$ that minimizes the energy-time product while both slowing the flow of blood to the capillaries and matching the supply and demand for oxygen. By our definition of a node as the volume of tissue that processes oxygen at a fixed rate, $T_{node}$ must be invariant. Table 1 shows the model prediction $T_{node} \propto u_0^{-1}N^{1-\frac{2}{D_r}}$.

Following~\cite{banavar10}, in the optimal case $u_0$ increases with organism mass, and therefore with $N$. See Section~\ref{subsec:appendixLengthMass} for the derivation that $u_0 \propto l_0 \propto N^{\frac{2}{3D_r} - \frac{2}{9}}$. Substituting this equation for $u_0$ into the equation for $T_{node}$ in Table 1, we find that $T_{node}$ is invariant with respect to $N$ when $D_r = \frac{24}{11} = 2.18$. The last column of Table 1 lists the scaling predictions given this value of $D_r$. We test this prediction for $D_r$ using data from~\cite{kolokotrones2010curvature} below.
...
nodes ($E_{net}$ and $E_{node}$), our energy-time minimization model provides an explanation for the curvature observed in
metabolic scaling in mammals~\cite{kolokotrones2010curvature}.  \red{This
work by Kolokotrones et al. showed that metabolic rate is elevated in both small and very large mammals, causing
...
plots, as shown in figure~\ref{fig:OrganismsPowerScaling}, it is important because the lack of a single scaling exponent calls into question the scaling models that purport to explain a universal scaling exponent.}
...
that is linear in the number of nodes.  We tested the predicted value of $D_r =
...
\red{To compare this theoretical prediction to the data, we fit $D_r$ in
...
\begin{figure}[!h] \centering
  ...
  \caption{\red{The energy-time minimization model predicts metabolic
    ...
    are plotted, showing slight but theoretically important curvature in the scaling of metabolic rate vs. mass of mammals.  The West et
    al.~\cite{west97} $3/4$ scaling prediction is shown in blue, and
...
$N^{-1/D_l}$). } The width of the smallest wires, 
...
prediction (Eq. \ref{eq:Power}) is shown in blue, and the best fit line is shown in red.}}
\label{fig:power}
\end{figure}
...
\caption{\red{The energy-time minimization model predicts how throughput
 ...
 prediction  (Eq. \ref{eq:Performance}) is shown as a solid blue
  line. The dotted green line shows an alternative prediction if throughput were 
  bound by the nodes (switching speed) rather than the network. The
  red dashed line is the best fit to the data.}}
...
computational systems spanning an enormous range of sizes.   Here, we 
presented a unified model, which predicts scaling relationships for both mammals and
computer chips by simultaneously minimizing energy dissipation and delivery time. 
%gives
%unified model, accounting for how both natural and artificial
%networks have evolved. 
This broad framework highlights the similarities and
...
delivery time, e.g., \cite{west97,RIVER}.  Here we extend that work by considering energy and time
simultaneously and investigating the tradeoffs between them.}

\red{This theoretical model can be used to make testable predictions, both
for biology and for technological systems.  In biology, Section
\ref{sec:bio-predictions} extends previous predictions to explain the
observed curvature in metabolic scaling of mammals by including an
energy dissipation term.  Other studies have interpreted this
deviation as the `non-universal' character of metabolic scaling.  The
framework presented here accounts for this deviation, and further, it explains the
elevated metabolic rate of largest mammals not addressed in
previous models.  
In computation, the unified model predicts well Rent's exponents, active power consumption and chip performance over 40 years of chip design.
...
artificial designs due to physical constraints despite the obvious
...
example, Ref.~\cite{newberry2015testing} did not find evidence for a
...
vessels, but Ref.~\cite{newberry2015testing} find values for $D_r$
...
computation. For mammals we give the first quantitative prediction
...
\red{Innovations in chip design mimic innovation in bacteria evolution:} The chip scaling described above
...
have emerged that optimize against physcial constraints.  Just as
...
chip. Like bacteria, however, there are limits to how far this process
can go.  There are no elephant-sized bacteria, and there will be no
silicon-based single core chips with quadrillions of transistors.

\red {Single core chips scale like unicellular protists:} Historical chip scaling mimics the linear relationship between performance and
...
packed into larger cells \red{[FIXME: Add Delong cite and fig num here]}. As size continues to increase however, this
...
\red{Multi-core chips echo the transition to multicellularity:} Computer chips are currently undergoing
...
considerations\red{\cite{FIXME}}.  If chips follow biology, we can expect
that the most important future advances in chip
design will increase network efficiency, for example by using optical
networks.

\red{Chip scaling deviates from biological scaling in important ways:} There are also important differences between scaling of oxygen delivery in biology and information
...
as physical limits are reached. \red{FIXME[cite something to say that these
limits have been reached.  Recent paper from Nature?]}
...
\red{Decentralized designs and sociality:}  We now
