...
%supply matches demand for both systems. (NOTE: later we don't insist that $T_net = T_node$ for chips -- they have different scaling. Fix that.)
%***
...
\label{sec:organisms}
%**
%NOTE: explicitly define $l_0$ for biology -- length of service volume or $1/D_l$?
%
%Include: Under the pipelining and steady-state assumptions, $T_{net} = T_{node}$, i.e., supply matches demand for both systems. (ISSUE: later we don't insist that $T_{net} = T_{node}$ for chips -- they have different scaling. Explain why we set them equal for organisms and not chips.)
%**
...
1/Q$.  %(QUESTION: are we sure this isn't N/Q, which would make the scaling relationships below make much more sense?)
...
%We first consider the special case where $D_r = 3$, and the exponent in Eq.~\ref{eq:resistance} equals 0.
%Simplifying and ignoring constant terms gives  $R \propto l_0 \lambda^{-H} H $ or  $R \propto l_0 N^{-1} log(N)$ where $l_0$ was assumed constant in ~\cite{west97} but shown to scale with $N$ in ~\cite{banavar10}.
%
%When $D_r < 3$, the the exponent in Eq.~\ref{eq:resistance} is negative, resulting in $R \propto l_0 N^{-1} c$ where c is a small constant. When $D_r > 3$, the the exponent in Eq.~\ref{eq:resistance} is positive, resulting in $R \propto l_0 N^{-1} N^{(\frac{4}{3} - \frac{4}{D_r})}$ and simplifying: $R \propto l_0 N^{(\frac{1}{3} - \frac{4}{D_r})}$



%As $D_r$ increases from 3 to 12, $R$ increases from
...
%we keep  ~\cite{west97} assume $u$ is constant within and across organisms. ~\cite{banavar10} assumed $u$ is constant within organisms, but between organisms predicts $l_0 \propto u_0 \propto N^{(\frac{1}{D_l})^{1/3}} \propto N^{1/9}$. That prediction that is based on minimizing time only and so the same argument can not be used here. I think we should just leave $u_H$ as a variable and say something about it in the velocity assumptions section.

%Assuming that $u_H$ is independent of $N$ and $Q$ is equal at all levels of the
%network (steady state assumption) gives:
%$ Q \propto  N^{\frac{2}{D_r}}$.  

%**Sanity Check and recap**
%
%$R \propto l_0 N^{-1}$ for $D_r < = 3$ %ignoring the logarithmic term when $D_r <= 3$
%
%$R \propto l_0 N^{(\frac{1}{3} - \frac{4}{D_r})}$ for $D_r > 3$. %Mimimizing means $D_r < 3$ and small as possible or $D_r > 12$ and large as possible. 
%
...
%Substituting scaling relations for $R$ and $Q$ into  Eq. \ref{eq:bio-min} and ignoring logarithmic and constant terms gives

%
%(NOTE: Deal with this:
%West et al. predict that $E_{net}$ is
...
%$N E_{sys} \propto l_0 u_0 N  + N $ (Total energy dissipated through the network scales linearly with N except for that pesky $l_0 u_0$)




%where $D_r = 2$ in the upper pulsatile area preserving part of the network so $T_{net} \propto N^{0} $ and $D_r = 3$ in the lower Poiseuillle area increasing part of the network so $T_{net} \propto N^{1/3}$.
...
$E_{sys} \propto N$  (now Esys is dominated by $E_{node}$)% l_0 u_0 N^{(1/3)}$  

$T_{sys} \propto u_0^{-1} N^{-2/3}$

%$N E_{sys} \propto l_0 u_0 N^{(4/3)} $ 


%$E_{net} \propto RQ \propto l_0 N^{-1}  N^{\frac{2}{D_r}} \propto l_0 N^{1/3}$ 
%
%$E_{node} \propto N^0$ (by reference to WBE)
%
%$T_{node} \propto T_{net} \propto 1/Q \propto u_0^{-1} N^{1-2/D_r}\propto u_0^{-1} N^{1/3}$ 
%
%
%$E_{sys} \propto l_0 N^{4/3} \propto l_0 M^{1}$
%
%$T_{sys} \propto u_0^{-1} N^{1/3}$
