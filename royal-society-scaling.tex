...
has changed over evolutionary time. For this we turn to work by Delong et al., who
...
importance of time and energy, both in the network
network  (i.e.
minimizing time to deliver metabolites) and in the nodes (maximizing the
capacity of nodes). 
FIXME(is this sentence in or out?)
They also note significant decreases in energetic
...
chips with quadrillions (FXIME ?) of transistors.

2.  Chips have also mimicked the linear relationship between performance and
size (Figure FIXME XXX) seen in protists. Unicellular protists show linear increases
...
size. Thus, it is likely that networks will increasingly dominate on-chip power
...
continue for several more generations FIXME(CITATIONS). Second, the locality of
...
communication primarily local (FIXME: maybe cite george here?). Communication locality
...
extraordinary fractions of the available energy on earth FIXME(citations)---40\%
