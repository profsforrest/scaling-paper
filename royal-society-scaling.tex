...
\begin{sciabstract}
  ...
  and time, our analysis explains
...
%In order to maximize computation speed and minimize
%power consumption, 
...
\item {\bf Terminal units and service volumes:} Previous scaling models of biology posit that the service volume (the volume of tissue that has an invariant metabolic rate and is served by terminal units of the network) increases with
  ...
  the assumption in~\cite{banavar10} that the capillaries have fixed
...
% and terminal regions have thus decreased. 
...
whereas an alternative model~\cite{banavar10} maximized metabolic rate. Not surprisingly, scaling models that assume different optimization principles make different predictions~\cite{newberry2015testing}.  Our model
...
% I know MM prefers the propto here, but SF thinks it reads better
% this way.
...
%Following \cite{banavar10} and under the steady-state assumption
...
It it is evident that $E_{net}$'s scaling behavior depends on the value of $D_r$:
...
\begin{caseof}
  ...
  \frac{2}{D_r}}$}
\end{caseof}
\noindent We note that $D_r < 2$ is physiologically unrealistic
because cross-sectional area of the network would decrease toward the
capillaries, causing blood velocity to increase rather than slowing down to allow oxygen absorption at the nodes. To
...
% SF thinks we no longer show this case in the table.
%\noindent The predicted scaling relationships for $D_r = 2$ are also shown in Table 1.
...
$N^{\frac{1}{12}} + N$ \T \\
\cline{2-4}
%\multicolumn{4}{c}{} \\[-10pt] %Total Hack to make the table look decent
...
with the aggregate cross-sectional area of pipes is constant for all
hierarchical levels.  Earlier derivations showed that area-preserving
branching leads to the observed 3/4 power scaling of metabolic rate with body
size (e.g., \cite{west97, banavar10}).  
%In
...
%assumed in~\cite{banavar10}. 
Our analysis shows that
area-preserving branching is optimal from the perspective of the energy-time product.
...
=3$\footnote{To understand how pulsatile flow affects resistance in
  ...
  $D_r = 2$ minimizes resistance in the upper pulsatile region.}. Eq. \ref{eq:EnetOrg} also demonstrates that $E_{net}$ is minimized for larger values of $D_r$ (up to the discontinuity at $D_r =3$). However, there is a tradeoff with $T_{net}$,
...
However, in animal circulatory networks blood must slow before reaching capillaries to
reduce pressure on the walls of small vessels and allow oxygen to be
dissociated from hemoglobin in the capillaries.   Under this circumstance,
perfect area-preserving branching is infeasible, but energy-time 
minimization allows us to predict the optimal way for blood to slow.
The derivation in Section \ref{sec:AppendixOrg} leads to a
...
Additionally, we can calculate the exact value of $D_r$ that provides
optimal slowing.  Using our definition of a node as the volume of
tissue that processes oxygen at a fixed rate, $T_{node}$ is then
invariant. However, when $D_r = 2$, $T_{node}$ varies inversely with
$u_0$ (blood velocity through the capillary.)
Following~\cite{banavar10}, in the optimal case $u_0$ increases with
organism mass, and therefore with
$N$. Section~\ref{subsec:appendixLengthMass} shows that $u_0 \propto
l_0 \propto N^{\frac{2}{3D_r} - \frac{2}{9}}$. Substituting this
equation for $u_0$ into the equation for $T_{node}$ in Table 1, we
find that $T_{node}$ is invariant with respect to $N$ when $D_r =
\frac{24}{11} = 2.18$. We test this prediction  using data from~\cite{kolokotrones2010curvature}.
...
\noindent (Ssubsection~\ref{subsec:appendixLengthMass} gives details of the calculation).
While this model more complicated than the original $\frac{3}{4}$
scaling predicted by West et al.~\cite{west97} or the alternative models
proposed by Kolokotrones et al.~\cite{kolokotrones2010curvature},
these exponents arise from combining two scaling relationships: the
metabolic rate of the nodes and the metabolic power required to drive
the network.  (Note that we do not include the metabolic energy required to push oxygen
through the network to the nodes because this is supplied by the
coronary artery which delivers oxygen directly to the heart, bypassing the network itself.)
...
\caption{Performance vs. number of transistors. The
  ...
  line.  The dotted line shows predicted performance if switching
  speed of transistors dominates performance rather than the network.}
...
different metabolic requirements. As discussed above, $D_r$ varies
within the vascular network with area-increasing branching in smaller
...
substantively change the order-of-magnitude results presented here
with a simple unified model.
...
%we show that the optimal network design (i.e., $D_l=3$  and $D_r=2$), 
...
predictions will require incorporating
factors such as leakage power, which comprise an increasing fraction
of chips energy budgets.
...
delay in the nodes, while then number of nodes increases.
%, as well as allowing the dramatic increase in the number of
%transistors and wires in a given area. 
...
Scaling in computer chips and other systems that process
information rather than materials can provide insights into an
important biological evolutionary transition---the transition to
sociality.  Understanding and improving the flow of energy, materials
and information through human societies is one of the greatest
challenges facing science and engineering, and scaling analyses
provide an important perspective on this problem~\cite{moses2012beyond}.  The
analysis in Delong et al. does not address scaling relationships in
animal societies.  However, sociality is an important evolutionary
transition, reflected in the ecological dominance of humans and ants
whose networked systems transport both energy and information. These
social species have experienced great success, dispersing over vast
territories across the globe and capturing a large fraction of
available energy ~\cite{haberl2007quantifying,
  holldobler1990ants}. Recent evidence suggests that ant colonies and
human societies follow similar scaling relationships as individual
organisms~\cite{moses2003allometry, bettencourt2007growth,
  burnside2012human, hou2010energetic, waters2010allometric}.

In social animal systems and computing, networks are partially decentralized, for example, traffic flow within cities
~\cite{samaniego2008cities} and among ant
nests~\cite{flanagan2013fast}. Tainter
et. al.~\cite{tainter2003resource} argue that human and ant
societies succeed in part by exploiting ``low-gain'' energy
systems---those that have low concentrations of dispersed energy but are
ubiquitous.   
%Low-gain systems can be exploited by systems capable
%of processing and storing vast quantities of energy. 
Understanding the
forces that have driven the tremendous power and performance scaling
in computing may lend insights into how other technologies can exploit
similar scaling relationships. In particular, communication locality
in computation suggests an important strategy in the transition to
sociality---animal societies can escape the constraints of a
centralized distribution network by evolving systems for decentralized
transportation and modular communication.  For example, the transition to
solar energy might be seen as capitalizing on dramatic
technological performance improvements that are reminiscent of those that kept computer technology
on Moore's Law~\cite{farmer2015predictable}.  
% SF thinks the problem with fossil fuels is that they we are using
% them up, not that we are centralized.
%This line of thinking
%suggests that enormous gains can be achieved in energy delivery as
%increasingly powerful solar cells use dispersed solar energy
%locally, thereby escaping the centralized distribution overhead of the fossil
%fuel based economy.
...
metabolic landscape of technology and become tightly coupled with the
