...
\end{tabular}
\caption{Table of Scaling for various dimension values $D_r$.\label{tab:SummaryScalingPredictions}}
\end{table}
...
minimizing the energy0time product is area preserving, i.e. $D_r = 2$
...
West et al.\ argued that $R$ and $E_{net}$ are minimized by $D_r
=3$\footnote{To understand how pulsitile flow affects resistance in
  the upper part of the network, see \cite{west97}, which shows that
  $D_r = 2$ in the upper pulsatile region.}.  FIXME (also evident in the equation for $E_{net}$ in
Table XX).  Although $E_{net}$ is minimized for larger values of $D_r$ (up
to the discontinuity at $D_r =3$), there is a tradeoff with $T_{net}$,
which is minimized when $D_r =2$. As $D_r$ increases from 2 to 3,
delivery rates are slow more quickly than energy dissipation,
so the $D_r =2$ area-preserving case is optimal for minimizing the
energy-time product.

However, in animal circulatory
networks blood must slow before reaching capillaries in order to
...
metabolic scaling in mammals. Kolokotrones et al FIXME(and previous authors?)
...
%the network $E_{net}$, here 
By considering the energy dissipated in \emnph{both} the network and 
nodes ($E_{net}$ and $E_{node}$), together with earlier 
results from~\cite{banavar10}
($l_0 \propto N^{\frac{1}{9}}$ and $u_0 \propto
N^{\frac{1}{9}}$) and the common assumption that $N \propto
M^{\frac{3}{4}}$, we predict the expected basal
...
both terms, each with different scaling exponents, the model accounts well for the
...
This model provides a better fit (Mean Square Error) than
...
\begin{figure}[!h] \centering
  ...
  \caption{Energy-time minimization model predictions for power
    ...
    was fit to data from FIXME(citation) by minimizing the distance
...
We now calculate the scaling of $E_{node}$. FIXME(should we say
something about leakage power here, as in acknowledging that it exists
and saying why we ignore it?) For a single node, computation
...
that $D_w = 2$ is consistent with observed
Rent's exponents (FIXME: CITATIONS).  
...
Our scaling models makes two important predictions.  First, power
...
measured exponent was $0.495$, which agrees closely with our prediction of
...
$1$, suggesting that engineered designs slightly outperform approach the
...
(see Eqn FIXME(XX)), so performance has increased only linearly with transistor
number even though, in principle, transistors could process information more
quickly.  As in biology, performance cannot be
understood without considring the constraints of the network.
