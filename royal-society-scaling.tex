...
\begin{sciabstract}
  ...
  as computer designs transitioni to decentralized multi-core and
...
the energy cost in the network.  For example, in biology the vascular network of vertebrate
...
Here, we model vertebrates as compoesed of regions of tissue that receive oxygen
...
length of pipe between successive hierarchical levels and the physical
...
branches at each hierarchical level forming a perfect tree structure
(generally with $\lambda = 2$~\cite{SavageCompBio2015}, i.e., forming
...
where $p = \frac{1}{D_w}$ is the Rent's exponent.\footnote{Rent's rule is typically
  ...
  Therefore, we can rewrite Rentt's rule as $c_i = c_0 \times \lambda^{ip}$,
...
it travels from the aorta to the capillaries FIXME(CITE somebody). Scaling models
